...
Describe Connect-XoSession {
    BeforeAll {
        Mock -CommandName Connect-XoSession -MockWith {
            # This return the value passed to the Connect-XoSession parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values using named parameters' {
        ...
        It 'Should return a single object' {
            #$return = Connect-XoSession -Data 'value'
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Connect-XoSession } | Should -Not -Throw

            #Should -Invoke -CommandName Connect-XoSession -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Connect-XoSession
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Connect-XoSession').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Connect-XoSession -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Connect-XoSession -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Connect-XoSession -Data 'value' -WhatIf
