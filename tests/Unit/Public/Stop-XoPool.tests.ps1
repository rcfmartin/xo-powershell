...
Describe Stop-XoPool {
    BeforeAll {
        Mock -CommandName Stop-XoPool -MockWith {
            # This return the value passed to the Stop-XoPool parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Stop-XoPool } | Should -Not -Throw

            #Should -Invoke -CommandName Stop-XoPool -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Stop-XoPool
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Stop-XoPool').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Stop-XoPool -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Stop-XoPool -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Stop-XoPool -Data 'value' -WhatIf
