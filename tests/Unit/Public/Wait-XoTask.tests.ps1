...
Describe Wait-XoTask {
    BeforeAll {
        Mock -CommandName Wait-XoTask -MockWith {
            # This return the value passed to the Wait-XoTask parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Wait-XoTask } | Should -Not -Throw

            #Should -Invoke -CommandName Wait-XoTask -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Wait-XoTask
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Wait-XoTask').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Wait-XoTask -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Wait-XoTask -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Wait-XoTask -Data 'value' -WhatIf
