...
Describe Restart-XoPool {
    BeforeAll {
        Mock -CommandName Restart-XoPool -MockWith {
            # This return the value passed to the Restart-XoPool parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values using named parameters' {
        ...
        It 'Should return a single object' {
            #$return = Restart-XoPool -Data 'value'
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Restart-XoPool } | Should -Not -Throw

            #Should -Invoke -CommandName Restart-XoPool -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Restart-XoPool
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Restart-XoPool').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Restart-XoPool -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Restart-XoPool -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Restart-XoPool -Data 'value' -WhatIf
