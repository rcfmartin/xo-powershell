...
Describe Stop-XoVm {
    BeforeAll {
        Mock -CommandName Stop-XoVm -MockWith {
            # This return the value passed to the Stop-XoVm parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Stop-XoVm } | Should -Not -Throw

            #Should -Invoke -CommandName Stop-XoVm -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Stop-XoVm
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Stop-XoVm').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Stop-XoVm -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Stop-XoVm -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Stop-XoVm -Data 'value' -WhatIf
