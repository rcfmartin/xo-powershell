...
Describe Get-XoPif {
    BeforeAll {
        Mock -CommandName Get-XoPif -MockWith {
            # This return the value passed to the Get-XoPif parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Get-XoPif } | Should -Not -Throw

            #Should -Invoke -CommandName Get-XoPif -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Get-XoPif
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Get-XoPif').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Get-XoPif -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Get-XoPif -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Get-XoPif -Data 'value' -WhatIf
