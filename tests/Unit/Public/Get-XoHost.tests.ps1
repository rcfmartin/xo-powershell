...
Describe Get-XoHost {
    BeforeAll {
        Mock -CommandName Get-XoHost -MockWith {
            # This return the value passed to the Get-XoHost parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Get-XoHost } | Should -Not -Throw

            #Should -Invoke -CommandName Get-XoHost -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Get-XoHost
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Get-XoHost').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Get-XoHost -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Get-XoHost -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Get-XoHost -Data 'value' -WhatIf
