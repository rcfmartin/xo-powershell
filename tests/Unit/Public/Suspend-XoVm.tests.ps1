...
Describe Suspend-XoVm {
    BeforeAll {
        Mock -CommandName Suspend-XoVm -MockWith {
            # This return the value passed to the Suspend-XoVm parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values using named parameters' {
        ...
        It 'Should return a single object' {
            #$return = Suspend-XoVm -Data 'value'
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Suspend-XoVm } | Should -Not -Throw

            #Should -Invoke -CommandName Suspend-XoVm -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Suspend-XoVm
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Suspend-XoVm').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Suspend-XoVm -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Suspend-XoVm -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Suspend-XoVm -Data 'value' -WhatIf
