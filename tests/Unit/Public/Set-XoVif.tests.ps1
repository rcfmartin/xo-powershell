...
Describe Set-XoVif {
    BeforeAll {
        Mock -CommandName Set-XoVif -MockWith {
            # This return the value passed to the Set-XoVif parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Set-XoVif } | Should -Not -Throw

            #Should -Invoke -CommandName Set-XoVif -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Set-XoVif
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Set-XoVif').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Set-XoVif -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Set-XoVif -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Set-XoVif -Data 'value' -WhatIf
