...
Describe Get-XoVbd {
    BeforeAll {
        Mock -CommandName Get-XoVbd -MockWith {
            # This return the value passed to the Get-XoVbd parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Get-XoVbd } | Should -Not -Throw

            #Should -Invoke -CommandName Get-XoVbd -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Get-XoVbd
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Get-XoVbd').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Get-XoVbd -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Get-XoVbd -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Get-XoVbd -Data 'value' -WhatIf
