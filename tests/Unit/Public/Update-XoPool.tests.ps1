...
Describe Update-XoPool {
    BeforeAll {
        Mock -CommandName Update-XoPool -MockWith {
            # This return the value passed to the Update-XoPool parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values using named parameters' {
        ...
        It 'Should return a single object' {
            #$return = Update-XoPool -Data 'value'
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Update-XoPool } | Should -Not -Throw

            #Should -Invoke -CommandName Update-XoPool -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Update-XoPool
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Update-XoPool').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Update-XoPool -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Update-XoPool -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Update-XoPool -Data 'value' -WhatIf
