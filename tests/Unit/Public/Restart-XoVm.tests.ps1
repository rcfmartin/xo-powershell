...
Describe Restart-XoVm {
    BeforeAll {
        Mock -CommandName Restart-XoVm -MockWith {
            # This return the value passed to the Restart-XoVm parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values using named parameters' {
        ...
        It 'Should return a single object' {
            #$return = Restart-XoVm -Data 'value'
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Restart-XoVm } | Should -Not -Throw

            #Should -Invoke -CommandName Restart-XoVm -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Restart-XoVm
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Restart-XoVm').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Restart-XoVm -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Restart-XoVm -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Restart-XoVm -Data 'value' -WhatIf
