...
Describe Get-XoSchedule {
    BeforeAll {
        Mock -CommandName Get-XoSchedule -MockWith {
            # This return the value passed to the Get-XoSchedule parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values using named parameters' {
        ...
        It 'Should return a single object' {
            #$return = Get-XoSchedule -Data 'value'
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Get-XoSchedule } | Should -Not -Throw

            #Should -Invoke -CommandName Get-XoSchedule -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Get-XoSchedule
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Get-XoSchedule').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Get-XoSchedule -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Get-XoSchedule -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Get-XoSchedule -Data 'value' -WhatIf
