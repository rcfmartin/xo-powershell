...
Describe Set-XoVdi {
    BeforeAll {
        Mock -CommandName Set-XoVdi -MockWith {
            # This return the value passed to the Set-XoVdi parameter $PrivateData.
            $PrivateData
    ...
    Context 'When passing values over the pipeline' {
        It 'Should call the private function two times' {
            #{ 'value1', 'value2' | Set-XoVdi } | Should -Not -Throw

            #Should -Invoke -CommandName Set-XoVdi -Exactly -Times 2 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return an array with two items' {
            #$return = 'value1', 'value2' | Set-XoVdi
    ...
    Context 'When passing WhatIf' {
        It 'Should support the parameter WhatIf' {
            #(Get-Command -Name 'Set-XoVdi').Parameters.ContainsKey('WhatIf') | Should -Be $true
            1 | Should -Be 1
        }

        It 'Should not call the private function' {
            #{ Set-XoVdi -Data 'value' -WhatIf } | Should -Not -Throw

            #Should -Invoke -CommandName Set-XoVdi -Exactly -Times 0 -Scope It -ModuleName $dscModuleName
            1 | Should -Be 1
        }

        It 'Should return $null' {
            #$return = Set-XoVdi -Data 'value' -WhatIf
